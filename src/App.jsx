...
const App = () => {
    ...
    setGame((prev) => {
      ...
      if (payload.clue) {
        ...
        if (!Number.isInteger(numericCount) || numericCount < 1) {
          invalidCount = true;
          statusOverride = 'Claude response invalid: count must be a positive integer.';
        } else {
          ...
          if (numericCount > remainingTeamWords && remainingTeamWords > 0) {
            ...
            statusOverride = `Claude count exceeded remaining ${team} words; clamped to ${remainingTeamWords}.`;
          } else {
  ...
  const handleFakeClaude = ({ team, role }) => {
    ...
    try {
      ...
      if (!parsed) {
        throw new Error('Fake Claude response could not be parsed.');
      }
      ...
      setStatusMessage(statusOverride ?? 'Fake Claude response applied.');
    } catch (error) {
      console.error(error);
      setStatusMessage(`Fake Claude response failed: ${error.message}`);
    }
  };
  ...
  const requestClaude = useCallback(async ({ team, role, apiConfigOverride = null }) => {
    ...
    setIsBusy(true);
    setStatusMessage(`Contacting Claude for ${team} ${ROLE_LABELS[role]}...`);
    ...
    try {
      ...
      setStatusMessage(statusOverride ?? 'Claude response applied.');
    } catch (error) {
      console.error(error);
  ...
  const roleSelectionLabel = (team, role) => {
    ...
    return persona ? `Claude · ${persona.label}` : 'Claude';
  };
                      ...
                      {isBusy ? (
                        ...
                        <div className="claude-response__error">
                          ...
                          <p className="error-message">
                            Claude's response could not be parsed as valid JSON. The response may be too creative or unstructured.
                          </p>
                  ...
                  <div className="endgame-modal">
                    ...
                    <div className="endgame-notes">
                      ...
                      {game.claudeNotesLog?.length ? (
                        ...
                        <p className="helper">No Claude notes logged.</p>
                      )}
                    </div>
